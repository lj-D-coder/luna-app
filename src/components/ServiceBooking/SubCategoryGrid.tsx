...
export const SubCategory: FC<SubCategoryProps> = ({ categoryUrl, onIconClick }) => {
    ...
    const categoryName = categoryUrl.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        {subCategoryList.map((subCategory, index) => (
                            <div className="items-center text-center">
                                <div key={index} style={{ cursor: 'pointer' }} onClick={() => onIconClick(subCategory.subCategoryName)} className="w-[100px] h-[100px] text-center px-2 py-5 rounded-lg shadow-md border border-gray-200 hover:border-gray-400 transition duration-300 flex flex-col justify-center items-center">
                                    <img
